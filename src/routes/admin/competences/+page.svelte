...
{#each data.domains as domain}
  ...
  <div class="mb-8 bg-gray-900/50 rounded-xl overflow-hidden">
    <!-- En-tête domaine -->
    <button
    ...
    >
      ...
      <span class="text-gray-500">{expandedDomain === domain.id ? '▲' : '▼'}</span>
    </button>

    {#if expandedDomain === domain.id}
      <div class="px-5 pb-5 border-t border-gray-800 pt-4 space-y-5">
        ...
        {#each categories as cat}
          ...
          <div class="bg-gray-800 rounded-xl overflow-hidden">
            <!-- En-tête catégorie -->
            <div class="flex items-center justify-between px-4 py-3 bg-gray-750">
              <div>
                <span class="font-semibold text-sm text-orange-300">{cat.name}</span>
                {#if cat.description}
                  <span class="text-xs text-gray-500 ml-2">{cat.description}</span>
                {/if}
                <span class="text-xs text-gray-600 ml-2">({catSkills.length} compétence{catSkills.length !== 1 ? 's' : ''})</span>
              </div>
            ...
            <div class="divide-y divide-gray-700/50">
              {#each catSkills as skill}
                <div class="flex items-center justify-between px-4 py-2.5 {skill.active ? '' : 'opacity-50'}">
                  <div>
                    <p class="text-sm font-medium">{skill.title}</p>
                  ...
                  <div class="flex items-center gap-2 flex-shrink-0 ml-3">
                    ...
                    <form method="POST" action="?/assignSkill">
                      ...
                      <select name="category_id" onchange={(e) => (e.target as HTMLSelectElement).form?.submit()}
                        class="text-xs bg-gray-700 border border-gray-600 rounded px-2 py-1 focus:outline-none">
            ...
            <div class="px-4 py-3 border-t border-gray-700/50">
              <form method="POST" action="?/addSkill" class="flex gap-2">
                ...
                <input name="title" type="text" placeholder="Titre de la compétence" required
                  class="flex-1 bg-gray-700 border border-gray-600 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-orange-500" />
        ...
        {#if uncategorized.length > 0}
          <div class="bg-gray-800/50 rounded-xl overflow-hidden border border-dashed border-gray-700">
            <div class="px-4 py-2.5 border-b border-gray-700/50">
              <span class="text-xs font-medium text-gray-500">Compétences sans catégorie ({uncategorized.length})</span>
            </div>
            {#each uncategorized as skill}
              <div class="flex items-center justify-between px-4 py-2.5 {skill.active ? '' : 'opacity-50'} border-b border-gray-700/30 last:border-0">
                <div>
                  <p class="text-sm font-medium">{skill.title}</p>
                  {#if skill.description}
                    <p class="text-xs text-gray-500">{skill.description}</p>
        ...
        {#if addingCategoryFor === domain.id}
          <form method="POST" action="?/addCategory" class="flex gap-2 items-end bg-gray-800 rounded-xl p-4">
            <input type="hidden" name="domain_id" value={domain.id} />
            <div class="flex-1">
            ...
            <div class="flex-1">
              ...
              <input name="description" type="text" placeholder="ex: Axes, engrenages, poulies"
                class="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-orange-500" />
            </div>
            <button type="submit"
            ...
            <button type="button" onclick={() => addingCategoryFor = null}
              class="text-gray-500 hover:text-gray-300 text-sm px-3 py-2">
              Annuler
            </button>
          </form>
        {:else}
