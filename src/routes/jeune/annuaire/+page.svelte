<script lang="ts">
  ...
  const filtered = $derived(
    data.users.filter((u: (typeof data.users)[number]) => {
      ...
      const matchTeam =
        teamFilter === 'all' || (data.teamMap[u.id] ?? []).includes(teamFilter);
...
<div class="max-w-2xl mx-auto">
  ...
  <p class="text-sm text-gray-400 mb-5">Cliquez sur un membre pour lui envoyer un message.</p>
  ...
  {#if data.teams.length > 0}
    <div class="mb-5">
      <select
        ...
        class="w-full sm:w-auto bg-gray-900 border border-gray-800 rounded-lg px-4 py-2 text-sm text-gray-200 focus:outline-none focus:border-orange-500"
      >
  ...
  {#if filtered.length === 0}
    <div class="text-center py-12 text-gray-600 bg-gray-900 rounded-xl">
      {search || roleFilter !== 'all' || teamFilter !== 'all' ? 'Aucun résultat pour cette recherche.' : 'Aucun membre.'}
    </div>
  {:else}
    <div class="space-y-2">
