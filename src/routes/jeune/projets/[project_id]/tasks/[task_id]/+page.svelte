...
<div class="max-w-2xl mx-auto">
  <!-- Retour -->
  <a href="/jeune/projets/{data.task.project_id}" class="text-sm text-gray-500 hover:text-gray-300 mb-4 inline-block">
    ← Retour au projet
  </a>

  <h1 class="text-2xl font-bold text-orange-400 mb-6">Tâche #{data.task.order_num}</h1>
  ...
  <div class="mb-6">
    <span class="inline-flex items-center px-3 py-1.5 rounded text-sm font-medium
      {data.task.state === 'todo' ? 'bg-gray-800 text-gray-400' :
       data.task.state === 'in_progress' ? 'bg-blue-900/50 text-blue-400' :
       data.task.state === 'done' ? 'bg-green-900/50 text-green-400' : 'bg-purple-900/50 text-purple-400'}">
      {stateLabels[data.task.state]}
    </span>
  </div>
  ...
  <form method="POST" action="?/updateTask" class="space-y-6">
    ...
    <div>
      <label class="block text-xs text-gray-400 mb-2" for="state">Statut</label>
      <select
        id="state"
        name="state"
        class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-gray-200 focus:outline-none focus:border-orange-500"
      ...
      <div>
        ...
        <div class="flex flex-wrap gap-2">
          {#each data.task.skills as skill}
            <span class="text-xs px-3 py-1.5 rounded bg-gray-800 text-gray-400">
              {skill.skill_title}
            </span>
          {/each}
        </div>
