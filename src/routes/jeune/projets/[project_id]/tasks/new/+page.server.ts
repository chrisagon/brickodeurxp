...
import { createTask, getProjectById, getTeamByJeune } from '$lib/server/db';
...
export const load: PageServerLoad = async ({ locals, params, platform }) => {
  ...
  // Vérifier que le jeune fait partie de l'équipe du projet
  const team = await getTeamByJeune(db, locals.session.user.id);
  if (!team || team.id !== project.team_id) {
    error(403, 'Vous n\'êtes pas autorisé à ajouter des tâches à ce projet.');
  }
...
export const actions: Actions = {
  createTask: async ({ request, params, locals, platform }) => {
    ...
    try {
      ...
      throw redirect(303, `/jeune/projets/${params.project_id}`);
    } catch (err: any) {
